...
struct HomeView: View {
    ...
    init(navigateTo: @escaping (NavRoute) -> Void) {
        ...
        // ViewModel is initialized via onAppear with container
        _viewModel = StateObject(wrappedValue: HomeViewModel.__allocating_init_stub())
    }
    ...
    var body: some View {
        ...
        ScrollView {
            VStack(alignment: .leading, spacing: 0) {
                ...
                // MARK: - Kanji Study Modes
                kanjiStudyModesSection(state: state)

                // MARK: - Flashcard & Collection
                Spacer().frame(height: 8)
        ...
        .onAppear {
            if viewModel.uiState.isLoading {
                // Re-init with container on first appear
            }
        }
            ...
            HStack {
                VStack(alignment: .leading, spacing: 2) {
                    ...
                    Text("Unlock all modes, J Coins & more")
                        .font(KanjiQuestTheme.labelSmall)
                        .foregroundColor(Color(hex: 0xB8860B).opacity(0.8))
                }
                Spacer()
                Text("$4.99/mo")
                    .font(KanjiQuestTheme.titleMedium)
    ...
    private var actionButtonsRow: some View {
        HStack(spacing: 8) {
            ...
            .tint(KanjiQuestTheme.secondary.opacity(0.2))
    ...
    private func gradeMasterySection(mastery: [GradeMastery]) -> some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            ScrollView(.horizontal, showsIndicators: false) {
                HStack(spacing: 12) {
                    ForEach(mastery, id: \.grade) { m in
                        GradeMasteryBadgeView(mastery: m)
                    }
                }
            }
        }
    ...
    private var kanaPracticeSection: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            HStack(spacing: 12) {
                GameModeButtonView(
                    label: "Hiragana", subtitle: "Recognition",
                    modeColor: Color(hex: 0xE91E63), imageAsset: "mode-kana-recognition.png",
                    action: { navigateTo(.kanaRecognition(kanaType: "HIRAGANA")) }
                )
                GameModeButtonView(
                    label: "Katakana", subtitle: "Recognition",
                    modeColor: Color(hex: 0x00BCD4), imageAsset: "mode-kana-writing.png",
                    action: { navigateTo(.kanaRecognition(kanaType: "KATAKANA")) }
                )
            }
        }
    }
    ...
    private func radicalModesSection(state: HomeUiState) -> some View {
        HStack(spacing: 12) {
            GameModeButtonView(
                label: "Radicals", subtitle: "Free",
                modeColor: Color(hex: 0x795548), imageAsset: "mode-radical-recognition.png",
                action: { navigateTo(.radicalRecognition) }
            )
    ...
    private func kanjiStudyModesSection(state: HomeUiState) -> some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            HStack(spacing: 12) {
                GameModeButtonView(
                    label: "Recognition", subtitle: "Free",
                    modeColor: Color(hex: 0x2196F3), imageAsset: "mode-recognition.png",
                    action: { navigateTo(.recognition) }
                )
                PreviewableGameModeButtonView(
                    label: "Writing",
                    isPremium: state.isPremium,
                ...
                case .schoolGrade:
                    ForEach(state.allGrades, id: \.self) { grade in
                        ...
                        Text(labelText)
                            ...
                            .cornerRadius(6)
                            .onTapGesture {
                                if hasCollection { viewModel.selectGrade(grade) }
                            }
                    }
        ...
        case .hiragana:
            LazyVGrid(columns: columns5, spacing: 8) {
                ForEach(state.hiraganaList, id: \.id) { kana in
                    KanaGridItemView(
                        kana: kana,
                        collectedItem: state.collectedHiraganaItems[kana.id],
                        onClick: { navigateTo(.kanaRecognition(kanaType: "HIRAGANA")) }
                    )
                }
            }
        ...
        case .katakana:
            LazyVGrid(columns: columns5, spacing: 8) {
                ForEach(state.katakanaList, id: \.id) { kana in
                    KanaGridItemView(
                        kana: kana,
                        collectedItem: state.collectedKatakanaItems[kana.id],
                        onClick: { navigateTo(.kanaRecognition(kanaType: "KATAKANA")) }
                    )
                }
            }
        ...
        case .radicals:
            LazyVGrid(columns: columns4, spacing: 8) {
                ForEach(state.radicals, id: \.id) { radical in
                    RadicalGridItemView(
                        radical: radical,
                        collectedItem: state.collectedRadicalItems[radical.id],
                        onClick: { navigateTo(.radicalDetail(radicalId: radical.id)) }
                    )
                }
            }
        ...
        case .kanji:
            LazyVGrid(columns: columns5, spacing: 8) {
                ForEach(state.gradeOneKanji, id: \.id) { kanji in
                    KanjiGridItemView(
                        kanji: kanji,
                        practiceCount: state.kanjiPracticeCounts[kanji.id] ?? 0,
                        modeStats: state.kanjiModeStats[kanji.id] ?? [:],
                        collectedItem: state.collectedItems[kanji.id],
                        onClick: { navigateTo(.kanjiDetail(kanjiId: kanji.id)) }
                    )
                }
            }
        }
...
private struct KanjiGridItemView: View {
    ...
    var body: some View {
        ...
        let borderColor = collectedItem.map { Color(UInt($0.rarity.colorValue)) }
                    ...
                    VStack {
                        HStack {
                            ...
                            Text("Lv.\(item.itemLevel)")
                                ...
                                .foregroundColor(Color(UInt(item.rarity.colorValue)))
                                .padding(2)
                        }
                        Spacer()
                    }
...
// MARK: - HomeViewModel initialization stub
// The actual init requires AppContainer, which comes via EnvironmentObject.
// We use a factory pattern where the real ViewModel is created in onAppear.
extension HomeViewModel {
    /// Placeholder init for StateObject wrapper — replaced by container-based init
    static func __allocating_init_stub() -> HomeViewModel {
        // This will be replaced. The actual container-based init happens
        // when AppContainer is available.
        fatalError("HomeViewModel requires AppContainer. Use HomeViewWrapper instead.")
    }
}

/// Wrapper that creates HomeViewModel with container from environment.
